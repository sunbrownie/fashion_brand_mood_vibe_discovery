...
if not NOTEBOOK_DIR.exists():
    NOTEBOOK_DIR = ROOT / 'moodboards_creation'

OUTPUT_DIR = ROOT / 'moodboards_creation' / 'moodboards'
if not OUTPUT_DIR.parent.exists():
    OUTPUT_DIR = NOTEBOOK_DIR / 'moodboards'
...
def load_gemini_api_key() -> tuple[str, Path | None]:
    ...
    env_candidates = [
        ...
        ROOT / 'brand_vibe_milestone_feasibility' / '.env',
    ]
...
def load_category(filename: str, category: str) -> pd.DataFrame:
    """Takes a category CSV and returns brand rows with filled vibe columns."""
    df = pd.read_csv(FINAL_DIR / filename)
    df['category'] = category
    for col in AESTHETIC_COLS:
        if col not in df.columns:
            df[col] = ''
    df[AESTHETIC_COLS] = df[AESTHETIC_COLS].fillna('')
    if 'moodboard' not in df.columns:
        df['moodboard'] = ''
    df['moodboard'] = df['moodboard'].fillna('')
    return df[['brand_name', 'official_website', 'category'] + AESTHETIC_COLS + ['moodboard']]

clothes   = load_category('brands_clothes.csv',   'clothes')
shoes     = load_category('brands_shoes.csv',     'shoes')
bags      = load_category('brands_bags.csv',      'bags')
jewellery = load_category('brands_jewellery.csv', 'jewellery')

brands_df = pd.concat([clothes, shoes, bags, jewellery], ignore_index=True)
...
def slugify(name: str) -> str:
    """Takes a brand name and returns a safe filename stem."""
    s = re.sub(r'[^\w\s-]', '', name)

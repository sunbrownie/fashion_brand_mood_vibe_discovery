...
# Drop mass-market / fast-fashion / high-street labels outside the target dataset.
...
# Sportswear rows can also appear as model-number artifacts.
MASS_PREFIXES = ('new balance',)

# Navigation, category, and generic words that leaked in as brands.
...
# Non-fashion and wrong-entity rows.
...
# Multi-brand retailers, boutiques, and media platforms saved separately from the brand catalogue.
...
# Duplicate brand variants the basic de-dup misses.
DROP_DUPLICATES = {_norm(x) for x in [
    'Nudie', 'Nudie Jeans Co',
...
# Website corrections checked by hand.
...
# Category corrections checked by hand.
...
# Extra brand rows added by hand.
ADD_BRANDS = [
    # luxury
    ...
    # corrected names for truncated scrape rows
...
# Apply website and category overrides first.
...
block = df['official_website'].map(_dom).isin(DOMAIN_BLOCKLIST)
# Source-like rows are saved separately, then dropped from the brand catalogue.
...
# Hand-checked rows that are not already present.
...
print(f'added {len(add)} hand-checked rows (of {len(ADD_BRANDS)})', flush=True)
